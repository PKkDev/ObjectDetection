...
namespace YOLO4.Shared.Parser
{
    public class Yolo4OutputParser
    {
        ...
        private readonly float scoreThres = 0.5f;
        private readonly float iouThres = 0.5f;

        private readonly int[] shapes = new int[] { 52, 26, 13 };

        private readonly int anchorsCount = 3;

        public List<YoloV4Result> ParseOutputs(YoloV4OutputData modelPredict)
        {
            if (modelPredict.Identity == null)
                return new List<YoloV4Result>();

            List<float[]> postProcesssedResults = new List<float[]>();
            int classesCount = categories.Length;
            var results = new[] { modelPredict.Identity, modelPredict.Identity1, modelPredict.Identity2 };
                ...
                for (int boxY = 0; boxY < outputSize; boxY++)
                {
                    for (int boxX = 0; boxX < outputSize; boxX++)
                    {
                        for (int a = 0; a < anchorsCount; a++)
                        {
                            ...
                            float org_h = ImageNetSettings.imageHeight;
                            float org_w = ImageNetSettings.imageWidth;
        ...
        /// <summary>
        /// применяет функцию-сигмоиду, которая выводит число от 0 до 1
        /// </summary>
        ...
        private float box_area(float[] box)
        {
